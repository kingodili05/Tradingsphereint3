...
export function useAdminActions() {
  ...
  const sendWithdrawalNotification = async (withdrawalId: string, status: 'completed' | 'failed') => {
    try {
      ...
      console.error('[Withdrawal notification] Failed to send email:', err);
    }
  };
  ...
  const approveWithdrawal = async (withdrawalId: string) => {
    ...
    try {
      ...
      // Deduct from user balance (use available_balance for available_balance field)
      ...
      sendWithdrawalNotification(withdrawalId, 'completed');
  ...
  const rejectWithdrawal = async (withdrawalId: string) => {
    ...
    try {
      ...
      // Send email notification to user (non-blocking)
      sendWithdrawalNotification(withdrawalId, 'failed');
